...
template<typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, string rsf) const
{
    if(subRoot == NULL) return;

    rsf.append(to_string(subRoot->elem));
    rsf.append(" ");
    ...
    printPaths(subRoot->left, rsf);
    printPaths(subRoot->right, rsf);
...
template <typename T>
int BinaryTree<T>::sumDistances(const Node* subRoot, int level) const
{
    if(subRoot == NULL){
        return 0;
    }
...
bool BinaryTree<T>::isOrdered() const
{
    ...
    int array[1000];
    int size = 0;
    loadIntoArray(root, array, size);
    return isArrayOrdered(array, size);
}

template <typename T>
void BinaryTree<T>::loadIntoArray(const Node* subRoot, int* array, int& size) const
{
    // Your code here
    if(subRoot == NULL)
        return;

    loadIntoArray(subRoot->left, array, size);
    array[size] = stoi(to_string(subRoot->elem));
    size++;
    loadIntoArray(subRoot->right, array, size);
}

template <typename T>
bool BinaryTree<T>::isArrayOrdered(const int* array, int size) const
{
    bool acc = true;
