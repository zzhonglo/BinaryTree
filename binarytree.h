...
class BinaryTree
{
  ...
  private:
    ...
    void printPaths(const Node* subRoot, string rsf) const;
    int sumDistances(const Node* subRoot, int level) const;
    void loadIntoArray(const Node* subRoot, int* array, int& size) const;
    bool isArrayOrdered(const int* array, int size) const;
